...
export default function ModeratorCreateTafsirPage() {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (res.success) {
        ...
        setTimeout(() => Link.push("/dashboard/moderator/manage/tafsir"), 1000);
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-2xl mx-auto">
        <nav className="flex items-center gap-2 mb-6">
          <Link href="/dashboard/moderator/manage/tafsir" className="text-gray-500"><ArrowLeft className="w-4 h-4" /></Link>
          <h1 className="text-xl font-bold text-gray-900">Add Tafsir</h1>
        </nav>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <FormContainer onSubmit={onSubmit}>
            ...
            <FormTextarea name="summaryBn" label="Summary (Bangla)" rows={2} />
            <FormTextarea name="summaryEn" label="Summary (English)" rows={2} />
            <FormTextarea name="detailBn" label="Detail (Bangla)" rows={3} />
            <FormTextarea name="detailEn" label="Detail (English)" rows={3} />
            ...
            <button type="submit" disabled={isLoading} className="mt-4 w-full bg-teal-600 text-white py-2 rounded-lg hover:bg-teal-700 disabled:opacity-50">
              ...
              {isLoading ? "Adding..." : "Create Tafsir"}
            </button>
          </FormContainer>
        </div>
      </div>
