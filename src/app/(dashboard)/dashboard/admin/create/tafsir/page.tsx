...
import { TafsirSchema } from "@/schema/tafsirSchema";
import { ArrowLeft, Book, Save, Sparkles } from "lucide-react";
import Link from "next/link";
import { useState } from "react";
import { toast } from "sonner";

const CreateTafsirPage: React.FC = () => {
  const [createTafsir, { isLoading: isSubmitting }] = useCreateTafsirMutation();
    ...
    try {
      ...
      if (res.success) {
        toast.success("Tafsir created successfully!", {
          description: "Interpretation has been added to the database.",
        });
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-6 lg:p-12 font-poppins">
      <div className="max-w-6xl mx-auto space-y-12">
        <nav className="flex items-center justify-between pb-6 border-b border-gray-200">
          <Link href="/dashboard/admin/manage/tafsir" className="group flex items-center gap-2 text-gray-500 hover:text-teal-600 font-semibold uppercase text-xs tracking-widest transition-all">
            <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
            <span>Back to Tafsir</span>
          </Link>
        ...
        <header className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl lg:text-5xl font-black text-gray-900 tracking-tight">Create Tafsir</h1>
              <p className="text-gray-500 font-medium mt-2 text-lg">Add interpretation for a verse</p>
            </div>
          </div>
        </header>
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-gray-200/50 border border-gray-100 overflow-hidden relative">
          ...
          <div className="p-10 lg:p-16 relative">
            <FormContainer onSubmit={onSubmit} resolver={zodResolver(TafsirSchema)}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <aside className="lg:col-span-4 space-y-10">
                  <div className="space-y-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <FormInput name="reference" label="Reference" placeholder="Reference book" />
                    </div>
                  </div>
                  <div className="p-8 bg-teal-50/50 rounded-2xl border border-teal-100/50 space-y-4">
                    ...
                    <ul className="space-y-3">
                      {["Valid Ayah ID", "Clear heading", "Scholar attribution"].map((rule, i) => (
                ...
                <main className="lg:col-span-8 space-y-10">
                  <div className="space-y-8">
                    <div className="flex items-center gap-3">
                      <div className="h-1 w-12 bg-teal-600 rounded-full"></div>
                      <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest">Tafsir Content</h3>
                    </div>
                    <FormTextarea name="summaryBn" label="Summary (Bangla)" rows={3} placeholder="Brief summary in Bangla..." />
                    <FormTextarea name="summaryEn" label="Summary (English)" rows={3} placeholder="Brief summary in English..." />
                    <FormTextarea name="detailBn" label="Detail (Bangla)" rows={5} placeholder="Detailed explanation in Bangla..." />
                    <FormTextarea name="detailEn" label="Detail (English)" rows={5} placeholder="Detailed explanation in English..." />
                  </div>
        ...
        <footer className="text-center pb-12">
          <p className="text-xs font-black uppercase text-gray-400 tracking-widest">Almunji Tafsir Management</p>
        </footer>
      </div>
    </div>
  );
...
export default CreateTafsirPage;
